...
class User(db.Model, UserMixin):
    ...
    def __init__(self, email, password, first_name, last_name, role, active, confirmed_at):
        self.email = email
...
def users():
    ...
    user_list.sort(key=lambda x: x.email)
...
def create_user():
    user = User(request.form['email'], request.form['password'], request.form['first_name'], request.form['last_name'],
                request.form['role'], 'true', datetime.datetime.now())
    db.session.add(user)
    db.session.commit()
...
@app.route('/deactivate_user/id/<id>', methods=['GET'])
@login_required
...
@app.route('/edit_user/id/<id>', methods=['GET'])
@login_required
def edit_user(id):
    user = User.query.get(id)
...
@app.route('/update_user/id/<id>', methods=['POST'])
@login_required
